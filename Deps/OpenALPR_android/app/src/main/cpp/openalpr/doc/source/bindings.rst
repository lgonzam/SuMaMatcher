...
.. code-block:: c++ 
   :linenos:
    ...
    alpr::com.openalpr.jni.AlprResults results = openalpr.recognize("/path/to/image.jpg");

    // Iterate through the results.  There may be multiple plates in an image, 
    // and each plate return sthe top N candidates.
    for (int i = 0; i < results.plates.size(); i++)
    {
      alpr::com.openalpr.jni.AlprPlateResult plate = results.plates[i];
      std::cout << "plate" << i << ": " << plate.topNPlates.size() << " results" << std::endl;
      
        for (int k = 0; k < plate.topNPlates.size(); k++)
        {
          alpr::com.openalpr.jni.AlprPlate candidate = plate.topNPlates[k];
...
.. code-block:: java
   :linenos:
    ...
    import com.openalpr.jni.com.openalpr.jni.AlprPlate;
    import com.openalpr.jni.com.openalpr.jni.AlprPlateResult;
    import com.openalpr.jni.com.openalpr.jni.AlprResults;
    ...
    com.openalpr.jni.AlprResults results = alpr.recognize("/path/to/image.jpg");
    System.out.format("  %-15s%-8s\n", "Plate Number", "Confidence");
    for (com.openalpr.jni.AlprPlateResult result : results.getPlates())
    {
        for (com.openalpr.jni.AlprPlate plate : result.getTopNPlates()) {
